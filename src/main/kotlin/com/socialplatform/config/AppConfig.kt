...
data class AppConfig(
    ...
    companion object {
        fun fromEnv() = AppConfig(
            ...
            redis = env("REDIS_URL", null)?.let { Redis(it) },
            rabbit = env("RABBITMQ_URI", null)?.let { Rabbit(it, env("RABBITMQ_EXCHANGE", "social.events")) },            supabase = Supabase(env("SUPABASE_URL", "http://localhost:54321"), env("SUPABASE_ANON_KEY", "dev-anon"), env("SUPABASE_SERVICE_ROLE_KEY", "dev-service"), env("SUPABASE_JWT_SECRET", "dev-secret-change-me"), env("SUPABASE_STORAGE_BUCKET", "media")),
